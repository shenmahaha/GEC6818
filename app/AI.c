...
static inline bool has_neighbor(int x, int y)
{
  ...
  for (int i = 0; i < 8; i++)
  {
    ...
    if (!OUT_OF_BORDER(nx, ny) && gobang[nx][ny].q_color != BLANK)
      return true;
  }
  return false;
}
...
double negmax(STATE player, pos pre, int depth, double alpha, double beta)
{
  ...
  for (int i = 0; i < 15; i++)
  {
    for (int j = 0; j < 15; j++)
    {
      if (gobang[i][j].q_color != BLANK)
        continue;
      if (!has_neighbor(i, j))
        continue;
      gobang[i][j].q_color = player;
      p = (pos){i, j, player};
...
static inline shape get_shape_through_direction(STATE player, pos p, pos vec)
{
  ...
  while (!OUT_OF_BORDER(x, y) && gobang[x][y].q_color == player)
  {
    x -= vec.x, y -= vec.y;
  }

  if (!OUT_OF_BORDER(x, y) && gobang[x][y].q_color == BLANK)
    S.front_space = 1;
  else
    S.front_space = 0;

  if (OUT_OF_BORDER(x, y) || gobang[x][y].q_color != player)
    x += vec.x, y += vec.y;
...
#define DRAW_SHAPE                                                 \
  ...
  {                                                                \
    while (!OUT_OF_BORDER(x, y) && gobang[x][y].q_color == player) \
    {                                                              \
      S.buffer[S.len++] = gobang[x][y].q_color;                    \
      if (gobang[x][y].q_color == player)                          \
        S.player_pieces_cnt++;                                     \
      x += vec.x, y += vec.y;                                      \
    }                                                              \
  } while (0)
...
bool player_win(STATE player, pos p)
{
  ...
  for (int i = 0; i < 8; i += 2)
  {
    ...
    if (s.player_pieces_cnt == 5 && s.len == 5)
      return true;
  }
  return false;
}
